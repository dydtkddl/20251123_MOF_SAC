...
# CHECKPOINT I/O
##############################################
PHASE1_CKPT = "../phase03_qmof_branch02_cmax_control_20251130/checkpoints_phase2/ckpt_ep1500_final.pt"  # <- 필요시 경로 수정
...
##############################################
POOL_DIR = "../mofs/train_pool_valid.coremof"
...
# CONFIG (Phase2)
##############################################
EPOCHS       = 1500          # Phase2 길이
BASE_STEPS   = 300
FINAL_STEPS  = 1000
HORIZON_SCH  = 500           # step 수 curriculum
...
WARMUP_TRANSITIONS    = 5_000   # Phase2에서도 새로 warm-up
...
for ep in range(EPOCHS):
    ...
    for step in tqdm(range(max_steps), desc=f"[EP {ep}]", ncols=120):
        ...
        # ----------------------------------------
        # 0) Mean action 제거 (global 평행이동 방지)
        # ----------------------------------------
        mean_action = action_arr.mean(axis=0, keepdims=True)  # (1, 3)
        action_arr = action_arr - mean_action                 # (N_atom, 3)
