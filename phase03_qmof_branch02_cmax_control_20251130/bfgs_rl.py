...
"""
RL vs BFGS Comparative Engine (Enhanced Logging + Full CSV Outputs)
=====================================================================
기능:
...
- 각 MOF별 상세 폴더 생성
- step-by-step energy/force/time CSV 생성
- RL vs BFGS energy plot 저장
- summary.csv에 모든 결과 종합
"""
...
def load_policy(checkpoint_path, obs_dim):
    ...
    dummy_rb = ReplayBuffer(obs_dim=obs_dim, max_size=1)
    agent = SACAgent(
        obs_dim=obs_dim,
        replay_buffer=dummy_rb,
        act_dim=3,
        device="cuda",
        lr=3e-4,
        gamma=0.995,
    ...
    logger.info("[LOAD] Policy restored successfully.")
    return agent
...
def bfgs_relax(atoms, save_dir):
    ...
    opt.attach(log_func, interval=1)
...
def main():
    ...
    logger.info(f"[DATA] Selected {len(selected)} structures.")
    ...
    with open(summary_path, "w") as f:
        f.write("name,rl_steps,bfgs_steps,rl_time,bfgs_time,rl_finalE,bfgs_finalE\n")
    ...
    for cif in tqdm(selected, desc="Comparing", ncols=120):
        ...
        with open(summary_path, "a") as f:
            f.write(f"{name},{rl_steps},{bfgs_steps},{rl_time:.4f},{bfgs_time:.4f},{rl_E[-1]},{bfgs_E[-1]}\n")

        logger.info(
            f"[DONE] {name} | RL={rl_steps} steps, BFGS={bfgs_steps} steps | "
            f"RL time={rl_time:.2f}s, BFGS time={bfgs_time:.2f}s"
        )

    logger.info("[END] Comparison completed.")
