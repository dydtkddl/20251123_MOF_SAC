...
class MOFEnv:
    ...
    def __init__(
        ...
        fail_penalty=15.0,
        # ---------- New: force 증가 패널티 ----------
        force_increase_scale=0.0,      # >0로 주면 활성화
        force_increase_threshold=0.05, # 5% 이상 증가 시만 벌점
    ):
        ...
        # New: force increase penalty hyperparameters
        self.force_increase_scale = force_increase_scale
        self.force_increase_threshold = force_increase_threshold
        ...
        self.last_force_increase_penalty = 0.0  # NEW: force 증가 벌점
    ...
    def reset(self):
        ...
        self.last_force_increase_penalty = 0.0
    ...
    def _set_last_reward_components(
        ...
        self.last_force_increase_penalty = float(force_inc_penalty)
    ...
    def step(self, action):
        ...
        # -----------------------------
        # 2-1) Force 증가 패널티 (NEW)
        # -----------------------------
        force_inc_penalty = 0.0
        if self.force_increase_scale > 0.0 and self.force_increase_threshold > 0.0:
            # ratio > 1 + threshold 인 부분만 벌점
            ratio = new_norm / old_norm  # old_norm, new_norm >= 1e-12
            thr = 1.0 + self.force_increase_threshold
            excess = np.maximum(ratio - thr, 0.0)  # (N,)

            if np.any(excess > 0.0):
                force_inc_vec = self.force_increase_scale * excess
                reward -= force_inc_vec
                force_inc_penalty = float(np.mean(force_inc_vec))
            else:
                force_inc_penalty = 0.0
        ...
        if delta_COM > self.com_threshold:
            ...
            self._set_last_reward_components(
                ...
                fail_penalty=0.0,
                terminal_bonus=0.0,
                force_inc_penalty=force_inc_penalty,
                reward=reward,
            )
